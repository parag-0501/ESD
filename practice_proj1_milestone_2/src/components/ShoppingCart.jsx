...
function ShoppingCart() {
  ...
  function calculateTotalPrice() {
    ...
    for (let i = 0; i < productsInShoppingCart.length; i++) {
      totalPrice += productsInShoppingCart[i].price *83* productsInShoppingCart[i].quantity; 
    }
    return totalPrice;
  }
        ...
        <>
          {productsInShoppingCart.map((eachProduct, index) => (
            <div id="single-cart-container" key={index}>
              ...
              <div id="price">
                <span id="rupee-span">₹</span>
                <span id="price-span">{eachProduct.price * 83 * eachProduct.quantity}</span>
                <span
                  id="trash-icon"
